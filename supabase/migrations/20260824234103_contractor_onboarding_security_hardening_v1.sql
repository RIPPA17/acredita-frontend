...
create or replace function private.get_contractor_invitation_preview_impl(p_token text)
returns table(
  invitation_id uuid,
  invited_email text,
...
as $$
  select
    i.id,
    i.invited_email,
    coalesce(i.contractor_name, c.name),
    coalesce(i.contractor_rut, c.rut),
    p.id,
    p.name,
    m.name,
    i.message,
...
begin
  ...
  if v_existing_membership is not null
     and (v_contractor_id is null or v_existing_membership <> v_contractor_id) then
...
create or replace function private.reject_contractor_invitation_impl(p_token text)
returns boolean
language plpgsql
security definer
set search_path = ''
...
revoke all on function private.get_contractor_invitation_preview_impl(text) from public, anon, authenticated;
revoke all on function private.accept_contractor_invitation_impl(text) from public, anon, authenticated;
revoke all on function private.reject_contractor_invitation_impl(text) from public, anon, authenticated;

grant execute on function private.create_contractor_invitation_impl(uuid,text,uuid,text,text,text) to authenticated;
grant execute on function private.get_contractor_invitation_preview_impl(text) to anon, authenticated;
grant execute on function private.accept_contractor_invitation_impl(text) to authenticated;
grant execute on function private.reject_contractor_invitation_impl(text) to authenticated;
...
as $$
  ...
  from private.create_contractor_invitation_impl(
    p_project_id, p_email, p_contractor_id, p_contractor_name, p_contractor_rut, p_message
  );
$$;
...
set search_path = ''
as $$
  select * from private.get_contractor_invitation_preview_impl(p_token);
$$;
...
set search_path = ''
as $$
  select * from private.accept_contractor_invitation_impl(p_token);
$$;
...
set search_path = ''
as $$
  select private.reject_contractor_invitation_impl(p_token);
$$;
