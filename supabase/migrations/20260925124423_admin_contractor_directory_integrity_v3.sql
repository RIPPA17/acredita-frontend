...
create unique index if not exists contratistas_normalized_rut_unique_idx
on public.contratistas ((private.normalize_rut(rut)))
where rut is not null and private.normalize_rut(rut)<>'';
...
begin
  ...
  if v_contact_name is null then
    raise exception using errcode='22023',message='Ingresa el nombre del responsable principal.';
  end if;
  if v_contact_email is null or v_contact_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then
    raise exception using errcode='22023',message='Ingresa un correo válido para el responsable principal.';
  end if;
