...
begin
  ...
  if old.status in ('completado','cancelado') then
    raise exception using errcode = '42501', message = 'El plan ya está cerrado y no puede ser reabierto por el contratista.';
  end if;
  ...
  if new.status not in ('en_progreso','completado') then
...
begin
  ...
  if p_status not in ('en_progreso','completado') then
    raise exception using errcode = '22023', message = 'Estado de plan no permitido para el contratista.';
  end if;
