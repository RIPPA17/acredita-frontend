...
begin
  ...
  new.rut := nullif(pg_catalog.btrim(coalesce(new.rut, '')), '');
  new.industry := nullif(pg_catalog.btrim(coalesce(new.industry, '')), '');
  new.phone := nullif(pg_catalog.btrim(coalesce(new.phone, '')), '');
  new.company_size := nullif(pg_catalog.btrim(coalesce(new.company_size, '')), '');
  new.message := nullif(pg_catalog.btrim(coalesce(new.message, '')), '');
...
begin
  ...
  if pg_catalog.char_length(pg_catalog.btrim(new.requester_name)) not between 2 and 200
     ...
     or pg_catalog.char_length(coalesce(new.details,'')) > 5000 then
