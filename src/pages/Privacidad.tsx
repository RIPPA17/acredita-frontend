        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-2xl border border-cream3 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="space-y-4 text-sm leading-6 text-gray-600">
              ...
              <p><strong className="text-navy">4. Bloqueo temporal.</strong> Cuando corresponda a una solicitud de rectificación, supresión u oposición, puedes pedir bloqueo temporal mientras se resuelve.</p>
            </div>

            <div className="mt-7 rounded-xl bg-cream2 p-4 text-sm leading-6 text-gray-600">
              <strong className="block text-navy">Contacto de privacidad</strong>
              contacto@acredita.cl
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-cream3 bg-white p-6 shadow-sm md:p-8">
            {sent ? (
              <div className="flex min-h-[420px] flex-col items-center justify-center text-center">
                <CheckCircle2 size={52} className="mb-5 text-green-600" />
              ...
              <form onSubmit={handleSubmit}>
                ...
                <input required maxLength={200} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="form-input mb-5 w-full rounded-lg border border-cream3 p-3" />

                <label className="mb-1.5 block text-sm font-medium">Correo de contacto</label>
                <input required type="email" maxLength={320} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="form-input mb-5 w-full rounded-lg border border-cream3 p-3" />

                <label className="mb-1.5 block text-sm font-medium">Tipo de solicitud</label>
                <select value={form.requestType} onChange={e => setForm({ ...form, requestType: e.target.value as RequestType })} className="form-input mb-5 w-full rounded-lg border border-cream3 p-3">
                  {REQUEST_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>

                <label className="mb-1.5 block text-sm font-medium">¿Qué datos o tratamiento quieres identificar?</label>
                <textarea required maxLength={1500} rows={3} value={form.scope} onChange={e => setForm({ ...form, scope: e.target.value })} className="form-input mb-5 w-full resize-none rounded-lg border border-cream3 p-3" placeholder="Ej.: datos de mi acreditación como trabajador en el proyecto…" />

                <label className="mb-1.5 block text-sm font-medium">Antecedentes adicionales <span className="font-normal text-gray-400">(opcional)</span></label>
                <textarea maxLength={5000} rows={4} value={form.details} onChange={e => setForm({ ...form, details: e.target.value })} className="form-input mb-5 w-full resize-none rounded-lg border border-cream3 p-3" />

                {['rectification', 'deletion', 'opposition'].includes(form.requestType) && (
                  <label className="mb-6 flex items-start gap-3 rounded-xl bg-cream2 p-4 text-sm leading-5 text-gray-700">
                    <input type="checkbox" checked={form.temporaryBlock} onChange={e => setForm({ ...form, temporaryBlock: e.target.checked })} className="mt-0.5" />
