...
type Estado = 'Aprobado' | 'En proceso' | 'Vencido/Bloqueado';
type DocEstado = 'Aprobado' | 'Rechazado' | 'Vencido' | 'En revisión' | 'Por vencer' | 'Pendiente';
...
export default function AcreditacionesTab({
  ...
  const rows: AcredRow[] = GLOBAL_CONTRATISTAS.flatMap(c =>
    c.proyectos.map(proyectoId => {
      const proyecto = GLOBAL_PROYECTOS.find(p => p.id === proyectoId);
      ...
      const projectWorkers = (c.trabajadores || []).filter(w => esTrabajadorAsignado(w, proyectoId, GLOBAL_PROYECTOS));
