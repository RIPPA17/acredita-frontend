...
function DecisionReviewsPanel({ rows, busy, setBusy, refresh, showToast }: { rows: DecisionReviewRow[]; busy: boolean; setBusy: (value: boolean) => void; refresh: () => Promise<void>; showToast: ToastFn }) {
  ...
  const decide = async (row: DecisionReviewRow, status: 'upheld' | 'overridden') => {
    ...
    try {
      ...
      showToast(status === 'overridden'
        ? 'Excepción humana registrada con vigencia temporal.'
        : 'Decisión automática confirmada por revisión humana.');
      await refresh();
    } catch (err) {
    ...
    <div key={row.id} className="rounded-xl border border-cream3 bg-white p-4">
      ...
      {!closed && (
        <div className="mt-4 border-t border-cream3 pt-4">
          <textarea value={reasons[row.id] || ''} onChange={event => setReasons({ ...reasons, [row.id]: event.target.value })} className="form-input w-full rounded-lg border border-cream3 px-3 py-2 text-[12.5px]" rows={2} maxLength={3000} placeholder="Fundamento de la revisión humana y evidencia revisada…" />
          ...
          <p className="mt-2 text-[11px] leading-4 text-gray-400">La excepción no aprueba ni modifica el documento: solo altera temporalmente la compuerta revisada y conserva trazabilidad.</p>
        </div>
      )}
    </div>
  );
  ...
  return <div className="space-y-5">
    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-[12px] leading-5 text-blue-900"><strong>Intervención humana.</strong> Revisa el estado automatizado, considera los antecedentes aportados y registra una decisión fundada.</div>
