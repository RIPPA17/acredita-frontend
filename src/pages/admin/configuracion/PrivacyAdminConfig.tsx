...
function DecisionReviewsPanel({ rows, busy, setBusy, refresh, showToast }: { rows: DecisionReviewRow[]; busy: boolean; setBusy: (value: boolean) => void; refresh: () => Promise<void>; showToast: ToastFn }) {
  ...
  const decide = async (row: DecisionReviewRow, status: 'upheld' | 'overridden') => {
    ...
    try {
      ...
      await resolveDecisionReview(row.id, {
        status,
        reason,
        overrideValue,
        overrideHours: hours[row.id] || 24,
      });
  ...
  const card = (row: DecisionReviewRow, closed = false) => (
    <div key={row.id} className="rounded-xl border border-cream3 bg-white p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
          ...
          <p className="mt-1 text-[12px] leading-5 text-gray-600"><strong>Motivo de la solicitud:</strong> {row.request_reason}</p>
          {row.requester_viewpoint && <p className="mt-1 text-[12px] leading-5 text-gray-500"><strong>Punto de vista aportado:</strong> {row.requester_viewpoint}</p>}
      ...
      {!closed && (
        <div className="mt-4 border-t border-cream3 pt-4">
          <textarea value={reasons[row.id] || ''} onChange={event => setReasons({ ...reasons, [row.id]: event.target.value })} className="form-input w-full rounded-lg border border-cream3 px-3 py-2 text-[12.5px]" rows={2} maxLength={3000} placeholder="Fundamento de la revisión humana: evidencia revisada, por qué se confirma o por qué procede una excepción…" />
          <div className="mt-3 flex flex-wrap items-end gap-2">
            <label className="text-[11px] font-semibold text-gray-500">Vigencia de excepción
              <select value={hours[row.id] || 24} onChange={event => setHours({ ...hours, [row.id]: Number(event.target.value) })} className="form-input mt-1 block rounded-lg border border-cream3 px-3 py-2 text-[12px] font-normal text-navy">
                <option value={4}>4 horas</option><option value={8}>8 horas</option><option value={24}>24 horas</option><option value={72}>72 horas</option><option value={168}>7 días</option>
              </select>
            </label>
          ...
          <p className="mt-2 text-[11px] leading-4 text-gray-400">La excepción no modifica ni elimina el documento original: sólo autoriza temporalmente la compuerta revisada y conserva la trazabilidad.</p>
        </div>
      )}
    </div>
  );
  ...
  return <div className="space-y-5">
    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-[12px] leading-5 text-blue-900"><strong>Intervención humana.</strong> Estas solicitudes permiten revisar un resultado automatizado, considerar el punto de vista del afectado y, cuando exista fundamento, autorizar una excepción temporal. La decisión debe basarse en evidencia y quedar trazada.</div>
