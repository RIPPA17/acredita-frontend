...
export default function DashboardTab({
  ...
  // Rejected documents are real correction work. Never invent a reason,
  // number of attempts, project or date when the source does not provide it.
  const waitingCorrectionList: Array<{
    id: number;
    empresa: string;
    proyecto: string;
    documento: string;
  ...
  GLOBAL_CONTRATISTAS.forEach(c => {
    (c.documentos || []).forEach(d => {
      ...
      const project = GLOBAL_PROYECTOS.find(p => (c.proyectos || []).includes(p.id));
      waitingCorrectionList.push({
        id: d.id,
        empresa: c.nombre,
        proyecto: project?.nombre || 'Proyecto no identificado',
        documento: d.nombre,
        motivo: d.motivo || d.observacion || 'Documento rechazado; requiere corrección.',
      });
    });
  });
    ...
    <div className="fade-in space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub text-gray-500 text-[13.5px]">Monitoreo y control de acreditaciones</p>
        </div>
      </div>
      ...
      <div className="ops-banner">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <div className="ops-banner-s">
              {conteo.critico} críticos · {conteo.atencion} en atención · {conteo.normal} normales
              {conteo.todos > 0 ? ' — empieza por lo más urgente' : ' — todo al día'}
            </div>
          </div>
        </div>
        <button
        ...
        >
          {primerPendiente ? 'Comenzar revisión' : 'Ver cola'} <ArrowRight size={16} />
        </button>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="stat-mini"><div className="stat-mini-n">{dynamicCola.length}</div><div className="stat-mini-l">Pendientes de revisión</div></div>
        <div className="stat-mini"><div className="stat-mini-n">{waitingCorrectionList.length}</div><div className="stat-mini-l">Esperando corrección</div></div>
        <div className="stat-mini"><div className="stat-mini-n" style={{ color: '#1e7a3c' }}>{aprobadosHoy}</div><div className="stat-mini-l">Aprobados hoy</div></div>
        <div className="stat-mini"><div className="stat-mini-n" style={{ color: '#b83232' }}>{rechazadosHoy}</div><div className="stat-mini-l">Rechazados hoy</div></div>
      </div>
      ...
      <div>
        <div className="flex flex-wrap justify-between items-end gap-3 mb-3">
          <div>
            ...
            <p className="text-[11.5px] text-gray-500 mt-0.5">Pendientes, vencimientos y correcciones ordenados por urgencia.</p>
          </div>
        ...
        <div className="card p-0 overflow-hidden">
          <div className="flex flex-wrap gap-2 px-4 py-3 border-b border-cream3">
            {filtros.map(f => (
              <button key={f.id} onClick={() => setFiltroCola(f.id)} className={`chip ${filtroCola === f.id ? 'active' : ''}`}>
                {f.label} <span className="chip-count">{conteo[f.id]}</span>
              </button>
            ))}
          </div>
          {colaVisible.length > 0 ? colaVisible.map(item => (
            <div key={item.key} className="qrow">
              <span className={`sev sev-${item.sev} w-[76px] px-0`}>{SEV_LABEL[item.sev]}</span>
              <div className="flex-1 min-w-0">
                <div className="text-[13.5px] font-semibold text-navy truncate">{item.empresa} — {item.documento}</div>
              ...
              <button onClick={item.onAccion} className="btn btn-ghost btn-sm shrink-0">{item.accion}</button>
            </div>
          )) : (
            <p className="text-center py-8 text-gray-400 text-[13.5px]">{filtroCola === 'todos' ? 'No hay nada pendiente. Todo al día.' : `No hay items en "${SEV_LABEL[filtroCola as Severidad]}".`}</p>
          )}
        </div>
      </div>
      ...
      <div>
        <div className="flex flex-wrap justify-between items-end gap-3 mb-3">
          <div>
            ...
            <p className="text-[11.5px] text-gray-500 mt-0.5">Últimos eventos reales de la operación.</p>
          </div>
          <select value={filtroActividad} onChange={e => setFiltroActividad(e.target.value)} className="form-input text-[11.5px] py-1 px-2 w-auto">
        ...
        <div className="card p-0 overflow-hidden bg-white/70">
          {actividadVisible.length > 0 ? actividadVisible.map(a => {
            const estado = normalizarEstado(a.estado);
            const badge = estado === 'aprobado' ? 'b-green' : estado === 'rechazado' ? 'b-red' : estado === 'registrado' ? 'b-blue' : 'b-yellow';
            const label = estado === 'aprobado' ? 'Aprobado' : estado === 'rechazado' ? 'Rechazado' : estado === 'registrado' ? 'Registrado' : 'En revisión';
            return (
              <div key={a.id} className="arow cursor-pointer" onClick={() => setActividadSeleccionada(a)}>
                <span className="font-semibold text-navy w-[170px] shrink-0 truncate">{a.empresa}</span>
                <span className="flex-1 text-gray-600 truncate" title={a.detalle || a.documento}>{a.documento}{a.detalle ? ` · ${a.detalle}` : ''}</span>
          ...
          {actividadFiltrada.length > MAX_ACTIVIDAD && <div className="px-4 py-2.5 text-[11.5px] text-gray-400 border-t border-cream text-center">Mostrando {MAX_ACTIVIDAD} de {actividadFiltrada.length} eventos</div>}
        </div>
      </div>
    </div>
  );
