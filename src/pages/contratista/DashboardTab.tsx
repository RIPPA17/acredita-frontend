...
export default function DashboardTab({
  ...
  const solicitarRevisionHumana = async () => {
    ...
    try {
      const automatedState = reviewType === 'access'
        ? acceso.estado
        : accesoPago.pagoEstado;
      const result = await requestProjectDecisionReview({
        ...
        explanation: {
          ...
          accreditationState: estado,
          accessState: acceso.estado,
          accessDetail: acceso.detalle || accesoPago.motivoAcceso || null,
          paymentState: accesoPago.pagoEstado,
          paymentDetail: accesoPago.motivoPago || null,
        },
      });
      setReviewMessage(result.alreadyOpen ? 'Ya existe una revisión humana abierta para esta decisión.' : 'Solicitud de revisión humana enviada. Acredita revisará la evidencia y dejará una decisión trazable.');
      setReviewReason('');
      setReviewViewpoint('');
    } catch (err) {
      setReviewMessage(err instanceof Error ? err.message : 'No fue posible solicitar la revisión humana.');
    } finally {
      setReviewBusy(false);
    }
  };
    ...
    <div className="inicio2-page">
      ...
      <section className="inicio2-floating">
        ...
        {proyectoRecienIniciado ? <div className="inicio2-empty"><FileText size={34} /><strong>Proyecto recién iniciado</strong><p>Aún no hay documentos ni trabajadores cargados para {proyectoActual.nombre}.</p><div><button type="button" onClick={() => setActiveTab('subir')}>Subir primer documento</button><button type="button" onClick={() => { setActiveTab('trabajadores'); setShowAddWorkerModal(true); }}>Agregar trabajador</button></div></div> : <>
          ...
          {(acceso.estado === 'bloqueado' || accesoPago.pagoEstado === 'bloqueado') && (
              ...
              {reviewOpen && <div className="inicio2-card-body">
                <div style={{ display: 'grid', gap: 10 }}>
                  ...
                  <label style={{ fontSize: 12, fontWeight: 700 }}>¿Por qué debería revisarse?
                    <textarea className="form-input" rows={3} maxLength={2000} value={reviewReason} onChange={event => setReviewReason(event.target.value)} placeholder="Ej.: el documento corregido ya fue entregado, existe una excepción contractual o el estado no coincide con la evidencia…" style={{ display: 'block', width: '100%', marginTop: 5 }} />
                  </label>
                  <label style={{ fontSize: 12, fontWeight: 700 }}>Tu punto de vista o antecedente adicional <span style={{ fontWeight: 400 }}>(opcional)</span>
                    <textarea className="form-input" rows={2} maxLength={2000} value={reviewViewpoint} onChange={event => setReviewViewpoint(event.target.value)} placeholder="Agrega contexto que la persona revisora debería considerar." style={{ display: 'block', width: '100%', marginTop: 5 }} />
                  </label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                    ...
                    <small style={{ color: '#6f6f63' }}>La solicitud no elimina el bloqueo por sí sola. Una persona debe revisar y fundamentar la decisión.</small>
                  </div>
                  ...
                  {reviewType === 'access' && <small style={{ color: '#7a7a6a' }}>Una excepción de acceso a nivel empresa no habilita por sí sola a trabajadores con bloqueos individuales.</small>}
                </div>
              </div>}
            </section>
          )}
