...
const contractorAccessState = (contractor: Contratista, projectId: string, workers: Trabajador[]) => {
  ...
  const workerStates = workers.map(worker => calcularAccesoTrabajador(worker, projectId, contractor.id));
  if (company.accesoEstado === 'bloqueado' || workerStates.some(state => state === 'bloqueado')) return 'bloqueado' as const;
  if (company.accesoEstado === 'pendiente' || workerStates.some(state => state === 'pendiente')) return 'pendiente' as const;
  return 'habilitado' as const;
};
