    ...
    <div className="fixed inset-0 bg-black/50 z-[610] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-[980px] max-h-[calc(100vh-24px)] overflow-y-auto" onClick={event => event.stopPropagation()}>
        <header className="flex justify-between items-start p-5 border-b">
          <div><h3 className="font-semibold text-navy text-lg">{readOnly ? 'Historial del activo' : 'Gestión operacional'} · {asset.identifier}</h3><p className="text-xs text-gray-500">{asset.name} · Estado automático: {asset.status} · Acceso {readOnly ? 'histórico' : asset.accessAllowed ? 'habilitado' : 'no habilitado'}{asset.retirementReason ? ` · Retiro: ${asset.retirementReason}` : ''}</p></div>
        ...
        {loading ? <div className="p-6 text-sm text-gray-500">Cargando gestión operacional…</div> : <div className="p-5 space-y-6">
          ...
          <section className="border rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3"><UserRound /><div><h4 className="font-semibold text-navy">Operadores asignados</h4><p className="text-sm text-gray-500">Solo trabajadores activos, acreditados y con acceso habilitado pueden operar el activo. La vigencia nunca puede superar su contrato laboral.</p></div></div>
