...
export default function AssetsPanel({
  ...
  const enabled = activos.filter(item => item.accessAllowed).length;
  const readyToOperate = activos.filter(item => item.accessAllowed && item.activeOperators > 0).length;
  ...
  return <article className="mandante-proyectos-section-card mandante-proyectos-panel">
    ...
    <div className="mandante-proyectos-table-wrap"><table><thead><tr><th>Tipo</th><th>Identificación</th><th>Contratista / servicio</th><th>Documentación</th><th>Ingreso</th><th>Operación</th><th>Estado</th><th></th></tr></thead><tbody>{assets.map(asset => <tr key={asset.id}><td>{asset.type === 'vehiculo' ? <span className="inline-flex gap-2"><CarFront />Vehículo</span> : <span className="inline-flex gap-2"><Construction />{asset.type === 'maquinaria' ? 'Maquinaria' : 'Equipo'}</span>}</td><td><strong>{asset.identifier}</strong><small className="block">{asset.name}{asset.brand ? ` · ${asset.brand} ${asset.model || ''}` : ''}</small>{!asset.active && <small className="block text-gray-500">Retirado{asset.retiredAt ? ` · ${asset.retiredAt.slice(0, 10)}` : ''}{asset.retirementReason ? ` · ${asset.retirementReason}` : ''}</small>}</td><td>{contractorNames.get(asset.contractorKey) || asset.contractorKey}<small className="block">{services.find(item => item.id === asset.serviceKey)?.nombre || 'Sin servicio asignado'}</small></td><td><span>{asset.approvedDocuments}/{asset.totalDocuments} aprobados</span>{asset.nextExpiry && asset.active && <small className="block text-gray-500">Próximo vencimiento: {asset.nextExpiry}</small>}</td><td>{!asset.active ? <span className="text-gray-500">No operativo</span> : asset.accessAllowed ? <span className="inline-flex gap-1 text-green-700"><CircleCheckBig />Permitido</span> : <span className="inline-flex gap-1 text-red-700"><CircleAlert />No habilitado</span>}</td><td>{!asset.active ? <span className="text-gray-500">Histórico</span> : !asset.accessAllowed ? <span className="text-red-700">No operable</span> : asset.activeOperators > 0 ? <span className="inline-flex gap-1 text-green-700"><CircleCheckBig />{asset.activeOperators} operador{asset.activeOperators === 1 ? '' : 'es'} habilitado{asset.activeOperators === 1 ? '' : 's'}</span> : <span className="inline-flex gap-1 text-yellow-700"><CircleAlert />Sin operador habilitado</span>}</td><td>{asset.active ? <b className={`mandante-proyectos-badge ${asset.status === 'habilitado' ? 'green' : asset.status === 'bloqueado' ? 'red' : 'yellow'}`}>{statusLabel[asset.status]}</b> : <b className="mandante-proyectos-badge">Histórico</b>}</td><td><div className="flex gap-2"><button type="button" onClick={() => setSelectedAsset(asset)}>{readOnly || !asset.active ? 'Ver historial' : 'Gestionar'}</button>{!readOnly && asset.active && <button type="button" onClick={() => startEdit(asset)}>Editar ficha</button>}{!readOnly && asset.active && <button type="button" onClick={() => void retire(asset)} title="Retirar activo"><Archive /> Retirar</button>}</div></td></tr>)}</tbody></table></div>
