...
export default function AssetMatrixPanel({
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (saving || readOnly || !form.documentType.trim()) return;
    setSaving(true);
    try {
    ...
    <section className="border rounded-xl p-4 my-4">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">{readOnly ? 'Matriz histórica asociada al proyecto. Se conserva para consulta y trazabilidad.' : 'Define documentos por tipo, su vigencia y si bloquean acceso. Los requisitos retirados se conservan como historial.'}</p>
        </div>
        <button type="button" onClick={() => void load()} aria-label="Actualizar matriz"><RefreshCw /></button>
      </div>

      {!readOnly && <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-4">
      ...
      {readOnly && <div className="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800"><strong>Proyecto histórico · matriz solo para consulta.</strong> No se pueden agregar ni retirar requisitos de activos.</div>}
      ...
      {loading ? <p className="text-sm text-gray-500">Cargando matriz…</p> : items.length === 0 ? (
        ...
        <div className="mandante-proyectos-table-wrap"><table><thead><tr><th>Tipo</th><th>Documento</th><th>Vigencia</th><th>Criterios</th><th>Estado</th><th></th></tr></thead><tbody>
          {items.map(item => <tr key={item.id}><td>{typeLabel[item.type]}</td><td><strong>{item.documentType}</strong><small className="block text-gray-500">Obligatorio · bloquea acceso</small></td><td>{item.validityDays ? `${item.validityDays} días` : 'Sin plazo fijo'}</td><td>{item.checklist.length ? item.checklist.join(' · ') : 'Sin checklist adicional'}</td><td>{item.active ? <strong className="text-green-700">Activo</strong> : <span className="text-gray-500">Histórico{item.retiredAt ? ` · ${item.retiredAt.slice(0, 10)}` : ''}{item.retirementReason ? <small className="block">{item.retirementReason}</small> : null}</span>}</td><td>{!readOnly && item.active && <button type="button" onClick={() => void retire(item)} aria-label="Retirar requisito" title="Retirar requisito"><Archive /></button>}{readOnly && <span className="text-xs text-gray-500">Solo consulta</span>}</td></tr>)}
        </tbody></table></div>
      )}
    </section>
  );
