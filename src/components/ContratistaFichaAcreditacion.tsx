    ...
    <section className="ficha2-modal" role="dialog" aria-modal="true" aria-labelledby="ficha2-title" onClick={event => event.stopPropagation()}>
      ...
      <div className="ficha2-scroll">
        ...
        <div className="ficha2-metrics">
          ...
          <article className={acceso.estado === 'habilitado' ? 'ok' : acceso.estado === 'bloqueado' ? 'danger' : 'warning'}><LockKeyhole /><span>Acceso a faena</span><strong>{acceso.estado === 'habilitado' ? 'Habilitado' : acceso.estado === 'bloqueado' ? 'Bloqueado' : 'Parcial'}</strong><small>{acceso.detalle || 'Sin restricciones'}</small></article>
          <article className={pago.pagoBloqueado ? 'danger' : 'ok'}><WalletCards /><span>Pago</span><strong>{pago.pagoBloqueado ? 'Retenido' : 'Habilitado'}</strong><small>{pago.motivoPago || 'Sin bloqueos de pago'}</small></article>
        </div>
